class ChatApp {
    ...
    async sendMessage() {
        ...
        const hasAttachmentTokens = /\$[a-zA-Z0-9]+\$/g.test(this.chatInput.value);
        if (!content && !hasAttachmentTokens) return;
        ...
        const attachmentIdMatches = this.chatInput.value.match(/\$([a-zA-Z0-9]+)\$/g);
        const attachments = [];
    ...
    async uploadAndGenerateID(file) {
        ...
        try {
            ...
            // add ID token to chat input (kept for resolution on send, but not visually shown)
